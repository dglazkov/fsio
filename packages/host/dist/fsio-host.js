...
import { FrameType, frameTypeName, encodeFrame, jsonFrame, decodeJson, parseFrames, now, CHUNK_RE, DIR_CHUNK_RE, b64urlDecode, RpcErrors, rpcResult, rpcError, PROTOCOL_VERSION, } from "@fsio/common";
...
for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === "--allow-shell")
        flags.allowShell = true;
    else if (a === "--poll")
        flags.poll = Number(args[++i]);
    else if (a === "--hot")
        flags.hot = Number(args[++i]);
    else if (a === "--no-watch")
        flags.watch = false;
    else if (a === "--fresh")
        flags.fresh = true;
...
function adoptSession(id) {
    ...
    if (status && status.state === "exited") {
        ...
        if (now() - (status.t ?? 0) > 60_000)
            removeSessionDir(s, "stale");
        return;
    }
    ...
    switch (method) {
        ...
        default:
            if (isRequest)
                ...
                log(`session ${s.id}: unknown notification ${method}`);
    }
}
...
if (flags.poll > 0)
    setInterval(scheduleScan, flags.poll);
...
//# sourceMappingURL=fsio-host.js.map