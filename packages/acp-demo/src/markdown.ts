// A small markdown parser for what the agent says.
//
// It lives in `src/` beside `paths.ts`, for the reason that file gives: this
// is browser-side logic with a security sentence attached, and a claim like
// that should be tested in Node on every push rather than only in a
// cooperative browser loop.
//
// **The sentence: agent text never becomes markup.** This parser emits a
// token tree and never an HTML string; the renderer (`web/markdown.ts`)
// turns that tree into lit templates, so every piece of agent-authored text
// lands in a text binding that lit escapes. That is not belt-and-braces
// paranoia about a chat bubble — the page holding this text also holds a
// readwrite `FileSystemDirectoryHandle` for the human's folder, and the text
// is model output that the folder's own contents can influence (an agent
// summarizing a file is quoting whatever that file says). `unsafeHTML` here
// would put a filesystem capability behind a string, so there is no seam in
// this design where an HTML string exists to be sanitized or forgotten.
//
...
//     newlines inside a paragraph into spaces. In a chat transcript people
//     write newlines meaning newlines, so paragraphs keep them and the
//     renderer sets `white-space: pre-wrap`.
//   - **An unterminated fence is a code block, not literal text.** Output
//     streams in token by token, so ``` arrives long before its closing
//     fence. Treating the fence as open-to-end-of-input means a code block
//     renders as code while it is still being written, instead of flashing
//     as raw backticks and then reflowing.
 ...
 *  clickable. Refusing by allow-list is the same posture the helper takes
 *  toward agents (#6): naming what is permitted, never what is forbidden. */
