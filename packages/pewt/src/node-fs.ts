...
//
// `pewt` is a real fsio client — the same as the page is (D11: the client's
// FS dependency is a structural type, so anything shaped like the browser's
// handles works). This is that shape over `node:fs`, with temp-file +
// rename standing in for the File System Access API's atomic commit on
// close() (D2).
//
// **This is the third copy in the repo**, after packages/bench/src/fs-shim.ts
// and packages/actuator-demo/src/node-fs.ts. Two copies was already the
// signal to extract (PROCESS.md rule 6) and
// [#155](https://github.com/dglazkov/fsio/issues/155) is open on where it
// goes; a third is not the moment to answer that question sideways, from
// inside a skeleton, so it is copied and the count is written down here.
