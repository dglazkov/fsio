...
function assertP50(label, out) {
    ...
    if (!row)
        throw new Error(`${label}: no result row in output`);
    const p50 = Number(row.split("|")[6]);
    if (!(p50 > 0))
        throw new Error(`${label}: unparseable p50 in: ${row}`);
    if (p50 > P50_CEILING_MS)
        throw new Error(`${label}: p50 ${p50}ms exceeds the ${P50_CEILING_MS}ms ceiling — wakeup-strategy regression?`);
