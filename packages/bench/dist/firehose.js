...
import { FrameType, jsonFrame, parseFrames, chunkName, dirChunkName, DIR_CHUNK_MAX_BYTES, rpcRequest, rpcNotification, SPAWN_REQUEST_ID, } from "@fsio/common";
...
const spawnSpec = { kind: "shell", cmd: "/bin/sh", args: ["-c", cmd], pty: true, cols: 200, rows: 24 };
            ...
            if (buf.length > offset) {
                ...
                for (const f of frames)
                    if (f.type === FrameType.DATA)
                        text += Buffer.from(f.payload).toString("utf8");
            }
        }
        catch { }
