...
let openedWith: (value: unknown) => void;
...
export const args: Promise<unknown> = new Promise((resolve) => {
  openedWith = resolve;
});
...
export function connectTo(port: MessagePort, openArgs?: unknown): void {
  channel.attach(port);
  openedWith(openArgs);
}
