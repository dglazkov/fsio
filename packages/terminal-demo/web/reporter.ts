// Cooperative-verification reporter (#16 S4, trimmed from the workbench's:
// same files, same contract). Self-reports into
...
class Reporter {
  ...
  /** Injected by tabs.ts (avoids an import cycle): live tab summary so the
   *  agent side can read multi-tab state straight from report.json. */
  tabSummary: () => Record<string, unknown>[] = () => [];
