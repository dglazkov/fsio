...
- **B1 — client logic in Node (per push).** `FsioSession` needs
  `FileSystemDirectoryHandle` *semantics*, not a browser: a Node shim over
  real `fs` (temp+rename emulating the swap-file commit) runs the real
  client against the real host. Lands with #17's FS-surface extraction.
