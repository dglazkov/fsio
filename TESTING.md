...
| actuator demo ([#152](https://github.com/dglazkov/fsio/issues/152)) | the demo-private command queue and page-state reducer: commands remain pending until a page writes a result, cancellation gets its own one-writer file, tab mutations return stable identifiers, and invalid targets return the stable `tab_not_found` application code. IndexedDB transactionality and real folder application are browser-only and use the cooperative loop; `report.json` names IndexedDB as state owner and reports the page's current tabs | `packages/actuator-demo/src/test-{queue,model}.ts` | per push + cooperative loop |
