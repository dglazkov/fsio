...
| actuator channel ([#153](https://github.com/dglazkov/fsio/issues/153)) | the whole local-to-page channel with the browser played by twenty lines of Node: a command written by the real CLI code travels a real folder through a real `HostServer` and two registered kinds, and the page's receipt comes back as the CLI's exit code. Covers the four ways it does not work — no helper, no page, the page's own refusal (an app error, distinct from a channel one), and the page closing mid-command — plus takeover, where the newest page wins and the displaced one is told. Beside it, the parts that need no folder: the reducer (a click and a command are the same operation), argv parsing (every usage exit reachable without a process), and the router's correlation and timeouts. What is left for the cooperative loop is the page itself, not the transport under it | `packages/actuator-demo/src/test-{loop,model,args,router}.ts` | per push |
| pewter operations ([#165](https://github.com/dglazkov/fsio/issues/165)) | the host half of Pewter with no browser: what makes a folder a pewter and how one is found from a working directory, the project list, the extension bundler (rebuild-on-save by mtime, a `</script>` in the bundle that cannot end the tag early, a stylesheet link inlined but never one that climbs out of the extension), and the whole path end to end — the same `call()` the CLI runs, over a real host, answering with the operation's own refusal code. Since [#174](https://github.com/dglazkov/fsio/issues/174) that path has a second half: the operations only a page can answer, with a stand-in page attached to the folder over a real session — a command down one session and a receipt up another, the page's own refusal keeping its code, and the three ways an answer never arrives (no page, a page that closed, a page that did not answer in time). Plus the check that keeps two copies of one list honest: the `pewter` package's typed API and the host's operation table fail the build when they disagree, in either direction | `packages/pewt/src/test-{pewter,repos,bundle,ops,api,router,loop}.ts`, `packages/pewter/src/test-tabs.ts`, `packages/create-pewt/src/test-scaffold.ts` | per push |
| pewter runs ([#168](https://github.com/dglazkov/fsio/issues/168)) | a real child process over a real host: a declared script's output arrives in order and in the stream it was written to, its exit code is the run's exit code, it runs in the project's directory with `pewt` on its `PATH`, and the last line before the process dies still arrives (the race the in-band `end` frame removes). Plus the only thing standing between a page and a process: an undeclared script never starts, a project name that could climb out of `repos/` is refused, no at the host's terminal is a refusal that says so, and a host with no terminal to ask in denies and names `--allow-runs`. The question itself is asked through an injected asker — driving the real `readline` prompt needs a pty, which is what a probe did once by hand | `packages/pewt/src/test-run.ts` | per push |
| pewter shells ([#170](https://github.com/dglazkov/fsio/issues/170)) | a real pty over a real host, driven from the same functions `pewt shell` calls: what is typed reaches the shell and what it prints comes back, the exit code is the shell's own, `--repo` starts it in that project, the size travels and so does a later resize, and closing it stops what the host started. The pipe case (`echo pwd \| pewt shell`) drives `attachTerminal` against streams that are not a tty. Plus the refusals: a project that is not there, a `cwd` that climbs out of the folder (the host's own containment, D22), no at the terminal, no terminal to ask in — and `--allow-runs` **not** allowing a shell, which is the P3 split the two flags exist for. The prompt itself is asked through an injected asker; the real `readline` question, raw mode and SIGWINCH were driven by hand through two ptys | `packages/pewt/src/test-shell.ts` | per push |
| pewter agents ([#172](https://github.com/dglazkov/fsio/issues/172)) | a real ACP adapter over a real host, with the adapter a fixture written where `npm i` would leave one — so the lookup under test is the real one: the roster is read from the pewter's own `package.json` and `node_modules`, never from `PATH`, and it carries no path of its own. A message crosses whole in each direction, the adapter's exit code is the session's, `--repo` starts it in that project, junk on stdout is diverted and counted, and the agent is handed eight environment variables rather than yours (no `SSH_AUTH_SOCK`, `pewt` on `PATH`). Plus the refusals: no adapter at all, a name nobody lists, a declared-but-not-installed dependency, a project that is not there, no at the terminal, and `--allow-runs` **not** allowing an agent. One test pins a known gap rather than fixing it — after the agent exits its diagnostics are unreachable ([#98](https://github.com/dglazkov/fsio/issues/98)), so that test is the one that should start failing when #98 is settled | `packages/pewt/src/test-agent.ts` | per push |
| pewter channel ([#165](https://github.com/dglazkov/fsio/issues/165)) | the extension's end of the message channel over a real `MessageChannel` in Node: calls made before the port lands are queued rather than lost, two in flight get their own answers, a refusal arrives as a thrown error carrying the operation's code, and a frame from another build is dropped rather than guessed at. Since [#168](https://github.com/dglazkov/fsio/issues/168) a call can also carry events before its answer: a run's lines reach `onOutput` in order and the answer still arrives, an extension's own throwing callback does not take the answer down with it, and an event for a call that already ended is dropped. Since [#170](https://github.com/dglazkov/fsio/issues/170) it can also be *sent to* while it runs: a shell hands back a live handle, keystrokes and window sizes reach it, bytes printed before that handle arrived are not dropped, and nothing is sent to a call that has already been answered. [#172](https://github.com/dglazkov/fsio/issues/172) adds the same shape carrying somebody else's protocol: an agent's ACP messages cross whole in each direction, and the channel reads none of them | `packages/pewter/src/test-bridge.ts` | per push |
| extension pipeline (`npm run bridge-probe`) | the browser half that needs no folder and therefore no human: source bundled into one self-contained file, mounted in a sandboxed iframe with no `allow-same-origin`, announcing itself and reaching an API over a message channel — and, for an operation that streams, its output reaching the extension while the call is still running. Since [#170](https://github.com/dglazkov/fsio/issues/170) it also covers traffic the other way: an extension holding a live shell types into it and resizes it from inside the sandbox, after the call that opened it was made, and one holding an agent sends a whole ACP message and reads the answer back whole ([#172](https://github.com/dglazkov/fsio/issues/172)). An extension also asks for a tab ([#174](https://github.com/dglazkov/fsio/issues/174)) — the first call the page answers itself, and the check is that nothing about making it looks different. Headless, because nothing in it asks for a grant. The parent is a stand-in for the shell — the real one, its folder and its session are what `npm run pewter-rig` covers | `scripts/bridge-probe.mjs` | per push locally; on demand in CI terms |
| pewter rig ([#165](https://github.com/dglazkov/fsio/issues/165)) | the whole of Pewter in headed Chrome against a real `pewt serve`, one human click per run (F15): a scaffolded pewter, a real bundle through the folder, the shell's own grant and session, a real child process the extension asked for, a real pty it typed into ([#170](https://github.com/dglazkov/fsio/issues/170)), a real ACP adapter resolved from the pewter's own `node_modules` ([#172](https://github.com/dglazkov/fsio/issues/172)), and the verdict read back out of `.fsio/client/`. Since [#174](https://github.com/dglazkov/fsio/issues/174) it also drives the half that has no other instrument: `pewt tabs` typed in a terminal, forwarded by the host down the session the browser holds, answered in the browser, and back — plus exit 4 measured before the browser exists, which is the one moment "no host" and "no page" are unambiguously different. Its host runs with `--allow-runs --allow-shells --allow-agents`, because a host spawned by a script has no terminal to ask in and would otherwise deny all three — three flags rather than one, which is the P3 split the rig has to opt into by name. **The Chrome profile lives beside the pewter, never inside it** — Chrome will not mint a File System Access handle for a directory containing its own user-data-dir, and it declines by leaving `getAsFileSystemHandle()` pending forever | `npm run pewter-rig` (`scripts/pewter-rig.mjs`) | on demand: when a browser-touching change lands |
...
| acp kind (#18) | the framing contract in both directions — one DATA frame is one whole ACP message however the agent chunks its writes, junk on stdout diverted not delivered, a two-message frame refused — plus the session facts a page reads (state posture, synthesized env with the ssh-agent socket absent), the allow-list refusal, and the agent's exit taking the kind's methods with it (#98). Driven against a fixture agent: no installed agent, no model key, no network. This demo confines nothing ([#145](https://github.com/dglazkov/fsio/issues/145)), so the spawn result is asserted to carry **no** `sandboxed`/`confinement`/`profile` field at all — absent rather than false, because a page rendering "NOT sandboxed" is still making a claim about a wall that does not exist | `packages/acp-demo/src/test-{framing,acp-kind}.ts` | per push |
...
| the helper's URL hints ([#124](https://github.com/dglazkov/fsio/issues/124)) | the rule the first-run shortcut rests on: **a hint is never load-bearing**. The helper opens the page with the folder it serves and the agent it would drive in the query, and every case here is either "a malformed hint becomes nothing" or "the bare URL is a supported way to arrive" — because a param the page *needs* would be a side channel (P2) rather than a saved gesture. A basename long enough to stop being a button label, a path-shaped one, a flag-shaped agent name and an unknown param invented by whoever shared the link all resolve to nothing; the helper is held to the same shape check, so a hint it cannot send is one the page would have dropped. The one param deliberately absent is any claim about what the agent may do — the URL is the one input to this page a stranger writes, and a safety sentence a stranger can author is not a safety sentence | `packages/acp-demo/src/test-launch.ts` | per push |
