...
| pewter operations ([#165](https://github.com/dglazkov/fsio/issues/165)) | the host half of Pewter with no browser: what makes a folder a pewter and how one is found from a working directory, the project list, the extension bundler (rebuild-on-save by mtime, a `</script>` in the bundle that cannot end the tag early, a stylesheet link inlined but never one that climbs out of the extension), and the whole path end to end — the same `call()` the CLI runs, over a real host, answering with the operation's own refusal code. Plus the check that keeps two copies of one list honest: the `pewter` package's typed API and the host's operation table fail the build when they disagree, in either direction | `packages/pewt/src/test-{pewter,repos,bundle,ops,api,loop}.ts`, `packages/create-pewt/src/test-scaffold.ts` | per push |
