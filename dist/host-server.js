...
import { FrameType, frameTypeName, encodeFrame, decodeJson, parseFrames, now, CHUNK_RE, DIR_CHUNK_RE, OUT_LOG_RE, segName, b64urlDecode, RpcErrors, rpcResult, rpcError, PROTOCOL_VERSION, CAPABILITIES, } from "@fsio/common";
...
const echoSafe = (s) => s.replace(/\p{C}/gu, "").slice(0, 64);
const within = (root, p) => {
...
class Session {
    ...
    id;
    dir;
    spawn = null; // params from the JSON-RPC request in spawn.json
    spawnId = null; // request id to answer (null = legacy bare spec)
    spawnAnswered = false;
    started = false;
    ...
    nextInSeq = null; // discovered from the smallest chunk present
    ...
    root = null;
    workspace = null; // its name — the only half that may travel
    kindSession = null; // registered kinds (D13)
    watchers = [];
    retryTimer = null;
...
export class HostServer {
    ...
    constructor(opts) {
        ...
        this.workspaces =
            opts.workspaces ??
                ((name) => name === undefined || name === ownName
    ...
    refuseLiveHost() {
        ...
        throw new Error(`another fsio host looks live on ${this.fsioDir} (pid ${pid}, last heartbeat ${Math.round(ageMs)}ms ago). ` +
    ...
    ensureGitignore() {
        ...
        for (;;) {
            ...
            if (up === dir)
                return; // filesystem root: not a git repo
            dir = up;
        }
    ...
    archiveTranscript(s, why) {
        ...
        if (!logs.length)
            return; // nothing was ever said; keep no monument to it
    ...
    tryStart(s) {
        ...
        if (!raw)
            return; // not written yet; a watch event will re-trigger
    ...
    async decideAndStart(s, kind, info) {
        ...
        if (!this.running || s.done)
            return; // host or session closed while deciding
        if (!d.allow) {
    ...
    toPty(s, data) {
        s.ptyPending += data;
        if (s.ptyTimer)
            return; // a drain is already running; order is the queue's
    ...
    startShell(s) {
        ...
        try {
            ...
            p.on("error", (e) => {
                if (s.done)
                    return; // late-callback guard (see pty branch)
    ...
    processIncoming(s) {
        ...
        while (chunks.has(s.nextInSeq)) {
            ...
            if (chunk.data !== undefined)
                fs.rmdirSync(p); // consumption ack
            else
                fs.unlinkSync(p);
            s.nextInSeq++;
        }
    ...
    handleRpc(s, msg, t1) {
        ...
        if (method === undefined)
            return; // a response; host has no pending requests
...
//# sourceMappingURL=host-server.js.map