...
export const newFileId = () => `file-${Math.random().toString(16).slice(2, 10)}`;
const TYPES = {
...
export function viewerFor(type) {
    if (type === "image/svg+xml" || type === "application/json")
        return "text";
...
export const sizeText = (n) => n >= 1048576 ? `${(n / 1048576).toFixed(1)} MB` : n >= 1024 ? `${Math.round(n / 1024)} KB` : `${n} B`;
